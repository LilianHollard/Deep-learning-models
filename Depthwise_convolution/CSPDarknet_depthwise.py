...
def ConvDwBnSiLU(c1, c2, k, s, p):
    return nn.Sequential(
            nn.Conv2d(c1, c2, k, s, p, groups=c1),
            nn.BatchNorm2d(c2),
            nn.SiLU()
    )

def depthwise_block(c1, c2, k=3, s=1, p=1):
    return nn.Sequential(
        ConvDwBnSiLU(c1, c1, k, s, p),
...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        ...
        #rint(f"Focus time: {time.time() - t1}")
        
        t1 = time.time()
        x = self.firstC3(x)
        #rint(f"first c3 time: {time.time() - t1}")
        t1 = time.time()
        x = self.inter_conv_1(x)
        #rint(f"inter 1 time: {time.time() - t1}")
        t1 = time.time()
        x = self.secondC3(x)
        #rint(f"2 c3 time: {time.time() - t1}")
        t1 = time.time()
        x = self.inter_conv_2(x)
        #rint(f"inter time: {time.time() - t1}")
        t1 = time.time()
        x = self.thirdC3(x)
        #rint(f"3 c3 time: {time.time() - t1}")
        t1 = time.time()
        x = self.inter_conv_last(x)
        #rint(f"inter time: {time.time() - t1}")
        t1 = time.time()
        x = self.lastC3(x)
        #rint(f"last c3 time: {time.time() - t1}")"""
...
def test_1():
    ...
    macs, params = get_model_complexity_info(CSP, (3, 640, 640), as_strings=True,
